...
export function CostoMantencionMaquinaria({ total, top5, loading, error }: Props) {
  if (loading) {
    ...
    <div className="card">
      ...
      {top5.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold">Top 5 más caras del mes</p>
          {top5.map(c => (
            <Link
              key={c.maquinaria_id}
              to={`/maquinarias/${c.maquinaria_id}`}
            ...
            >
              <span className="font-mono font-semibold text-sm text-dark">{c.codigo}</span>
