...
interface FilterBarReportesProps {
  allData: VInspeccion[]
}
...
export function FilterBarReportes({ allData }: FilterBarReportesProps) {
  ...
  const hasActive =
    filters.fundo || filters.contrato || filters.conductor || filters.patente ||
    filters.desde || filters.hasta || filters.soloConProblemas || filters.search
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-4 sticky top-16 z-40">
      <div className="flex flex-wrap gap-2 items-end">
        <div className="min-w-[200px] flex-1">
          ...
          <input
            type="search"
            placeholder="Patente, conductor, comentario..."
            value={filters.search}
            onChange={e => setFilter('search', e.target.value)}
            className="input"
          />
        </div>
        <Select label="Fundo" value={filters.fundo} onChange={v => setFilter('fundo', v)} options={fundos} placeholder="Todos" />
        <Select label="Contrato" value={filters.contrato} onChange={v => setFilter('contrato', v)} options={contratos} placeholder="Todos" />
        <Select label="Conductor" value={filters.conductor} onChange={v => setFilter('conductor', v)} options={conductores} placeholder="Todos" />
        <Select label="Patente" value={filters.patente} onChange={v => setFilter('patente', v)} options={patentes} placeholder="Todas" />
        <div>
