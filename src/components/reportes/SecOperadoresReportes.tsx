...
export function SecOperadoresReportes({ data, operadoresNomina }: SecOperadoresReportesProps) {
  ...
  // Volumen de reportes por operador, coloreado según cumplimiento 5x5.
  const chartData = [...stats]
    .sort((a, b) => b.reportes - a.reportes)
    .map(s => ({ name: s.conductor, reportes: s.reportes, cumplimiento: s.cumplimiento }))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* Horizontal bar chart — volumen de reportes, color por cumplimiento */}
        <div className="card">
          <h4 className="text-sm font-semibold text-gray-700 mb-3">Volumen de reportes por operador</h4>
          <p className="text-xs text-gray-400 mb-3">Quién está levantando inspecciones — color según cumplimiento 5x5</p>
            ...
            <BarChart layout="vertical" data={chartData} margin={{ top: 4, right: 50, left: 10, bottom: 4 }}>
              ...
              <Tooltip
                content={({ active, payload, label }) => {
                  ...
                  const cumplimiento = (payload[0].payload as { cumplimiento: number }).cumplimiento
                  return (
                    <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-xs">
                      <p className="font-bold text-gray-900 mb-1">{String(label)}</p>
                      <p style={{ color: String(entry.color) }}>Reportes: <b>{String(entry.value)}</b></p>
                      <p className="text-gray-500">Cumpl. 5x5: {cumplimiento}%</p>
                    </div>
                  )
                }}
              />
              <Bar dataKey="reportes" radius={[0, 4, 4, 0]}>
